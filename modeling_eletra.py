...
class ElectraForSequenceClassification(ElectraPreTrainedModel):
    ...
    def forward(
        ...
        cur_batch_size = discriminator_hidden_states_1[0].size()[0]
        hidden_state = torch.randn(1*2,49,128) # [num_layers*num_directions,batch_size,num_hidden]
        ...
        #print('cls device:',cls_vec.device)
        ...
        input_for_rnn = input_for_rnn.view((-1,49,256))

        #print('middle of model')
        #print(input_for_rnn.size(),hidden_state.size())
        discriminator_hidden_states = self.gru(input_for_rnn,hidden_state)
        #print('disc device: ',discriminator_hidden_states[0].device)
        
        discriminator_hidden_states = (discriminator_hidden_states[0].to(my_device),)
        cls_vec = torch.unsqueeze(cls_vec,1)
        #print('cls device: ',cls_vec.device)
        tmp = self.fc_layer(torch.cat((cls_vec[0,0,:],discriminator_hidden_states[0][0,0,:],discriminator_hidden_states[0][0,48,:])))
        sequence_output = tmp
        sequence_output = sequence_output.to(my_device)
        #print('seq device: ',sequence_output.device)
        for idx in range(1,cur_batch_size):
            tmp = self.fc_layer(torch.cat((cls_vec[idx,0,:],discriminator_hidden_states[0][idx,0,:],discriminator_hidden_states[0][idx,48,:])))
        ...
        #print(discriminator_hidden_states_1[0].size())
