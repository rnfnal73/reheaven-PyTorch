...
class ElectraForSequenceClassification(ElectraPreTrainedModel):
    def __init__(self,config):
        ...
        self.fc_layer = nn.Linear(1536,256,bias=True)
        self.init_weights()
        '''
        @add_start_docstrings_to_callable(ELECTRA_INPUTS_DOCSTRING)
        @add_code_sample_docstrings(
    ...
    def forward(
        ...
        hidden_state = torch.randn(1*2,50,128) # [num_layers*num_directions,batch_size,num_hidden]
        if torch.cuda.is_available():
            hidden_state = hidden_state.cuda()
        #tmp_output = self.gru(discriminator_hidden_states_1[0],hidden_state)
        discriminator_hidden_states = self.gru(discriminator_hidden_states_1[0],hidden_state)
        #print(tmp_output[0].size())
        #a = tmp_output[0].view(64,50,2,768)
        #print(type(a),' ',a.size())
        #discriminator_hidden_states = self.fc_layer(tmp_output[0])
        #print(self.gru)
